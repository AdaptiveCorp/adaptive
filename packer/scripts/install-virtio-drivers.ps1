...
elseif ($ProductName -like "*2003*") {
    ...
    Write-Host ( "Unknown OS: $ProductName, using 2k22" )
    $VirtIO_OS = "2k22"
}

$VirtioDrive = $null
foreach ($d in @('D','E','F','G')) {
    if (Test-Path "$d`:\virtio-win-gt-x64.msi") {
        $VirtioDrive = $d
        break
    }
}
...
if (-not $VirtioDrive) {
    Write-Host "VirtIO drive not found"
    exit 1
}

$DriverPath = Get-Item "$VirtioDrive`:\*\$VirtIO_OS\$VirtIO_Arch"
...
if ([System.Environment]::Is64BitOperatingSystem) {
    & "$VirtioDrive`:\virtio-win-gt-x64.msi" /quiet /passive
}
else {
    & "$VirtioDrive`:\virtio-win-gt-x86.msi" /quiet /passive
}

& "$VirtioDrive`:\virtio-win-guest-tools.exe" /quiet /passive
