...
def wait_for_task_completion(proxmox, task_upid: str, timeout: int = 300, check_interval: int = 5):
    """
    ...
    while True:
        ...
        if elapsed > timeout:
            raise TimeoutError(f"[!] Timeout après {timeout}s en attendant la tâche {task_upid}")
...
def clone_vm_for_server(proxmox, project_id: int, server_id: int, template_id: int, db: Session):
    """
    ...
    server = db.query(orm_models.DBServer).filter(
    ...
    try:
        ...
        print(f"[>] Attente de la fin du clonage...")
