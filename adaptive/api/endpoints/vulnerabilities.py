...
def post_vulnerability(project_id: int, domain_id: int, vuln_id : int, request : VulnerabilityRequest,  db: Session = Depends(get_db)):
    ...
    applied_template = AppliedTemplate(project_id=project_id, template_id=vuln_template.id, domain_id=domain_id, params=param_req_str)
